...
class Host:
 def __init__(self, directory, version='0.3.0'):
  ...
  self.send({**HELLO,'host_version':version});self.registration=self.read();VALIDATOR.validate(self.registration)
  self.commands={x['name']:x for x in self.registration['commands']}
  self.send({**REGISTERED,'runyte':'>=0.3.0, <0.4.0','capabilities':self.registration['required_capabilities']})
 ...
 def read(self):
  ...
  msg=json.loads(line);VALIDATOR.validate(msg);return msg
...
class WireTests(unittest.TestCase):
 ...
 def test_browse_query_selection_and_stale_actions(self):
  ...
  old=h.revisions[view];self.good(h.invoke('activate',view,['0']));h.wait_jobs();self.assertEqual(h.views[view]['columns'][1]['label'],'name')
  self.assertIn('error',h.invoke('activate',view,['0'],revision=old))
  self.good(h.invoke('activate',view,['0']));self.assertIn('first',h.views[view]['rows'][1]['text'])
 ...
 def test_write_review_commit_and_rollback(self):
  self.connect();h=self.h;self.good(h.invoke('mode',h.active));self.good(h.submit({'confirmed':True}));h.wait_jobs()
 ...
 def test_sql_never_runs_when_review_cancelled(self):
  self.connect();h=self.h;self.good(h.invoke('mode',h.active));self.good(h.submit({'confirmed':True}));h.wait_jobs();self.good(h.invoke('query',h.active));b=list(h.buffers)[-1];h.buffers[b]='DELETE FROM items'
 ...
 def test_failed_write_clears_recovery_marker_after_rollback(self):
  self.connect();h=self.h;self.good(h.invoke('mode',h.active));self.good(h.submit({'confirmed':True}));h.wait_jobs();self.good(h.invoke('query',h.active));b=list(h.buffers)[-1]
 ...
 def test_reconnect_requires_explicit_buffer_reassociation(self):
  ...
  self.good(h.invoke('mode',catalog));self.good(h.submit({'confirmed':True}));h.wait_jobs()
 ...
 def test_empty_column_labels_in_queries_and_tables(self):
  ...
  self.good(h.invoke('activate',v,['0']));h.wait_jobs();self.assertEqual(h.views[v]['columns'][0]['label'],'(unnamed)')
 ...
 def test_wide_record_keeps_every_column_selectable(self):
  ...
  self.good(h.invoke('activate',v,['0']));rows=h.views[v]['rows']
  ...
  self.good(h.invoke('activate',v,['599']));self.assertEqual(h.views[h.active]['rows'][0]['text'],value)
 ...
 def test_control_character_table_title(self):
  ...
  self.connect();h=self.h;v=h.active
  self.good(h.invoke('activate',v,['0']));h.wait_jobs()
 ...
 def test_refresh_preserves_page_and_schema_shows_metadata(self):
  ...
  self.connect();h=self.h;v=h.active;self.good(h.invoke('activate',v,['0']));h.wait_jobs()
  ...
  self.assertIn('schema',h.views[v]['actions']);self.good(h.invoke('schema',v));h.wait_jobs()
